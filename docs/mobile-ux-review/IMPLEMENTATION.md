...
## Thread presentation increment — 2026-09-25

Mobile source1.0.64/code120 turns user prompts into right-aligned compact bubbles and places icon actions beneath messages. Assistant text stays unboxed with the Delta orb; no thumb ratings were added. Browser exploration sent one harmless text prompt in the local web session and confirmed the user row and Copy/Edit action names. [Thread](evidence/2026-09-25/thread-user.png) and [inline-edit](evidence/2026-09-25/thread-edit.png) captures show the solid header and edit field. Update was attempted once with a revised prompt; web PixelNano reported unavailable and the original thread remained visible while the revised text stayed in the editor. This was not a successful model reply, edited-thread replacement or regenerate verification.

Regenerate is scoped to the final plain Nano response and invokes only the model; SessionStore replaces a matching final response by session and turn ID after generation succeeds, preserving the old reply on generation or persistence failure. Prior plain response versions remain selectable. Edit now opens an inline field and uses a model-only response to replace the selected user turn and later plain-text turns atomically; failed generation leaves the old thread intact. The normal DeltaAgent path, tool actions and confirmation gate are never replayed. Live and tool/media replies expose no revision actions. Group kind/participant fields are backward-compatible persisted metadata only; the UI and harness still do not route multi-agent replies.

## Optional Jev settings ownership — 2026-09-25

The user corrected the optional TypeSafe/Jev placement: Connections describes MCP servers, external services, feeds and relays; Settings S09 owns the opt-in and cloud disclosure; Settings S02 Keys owns the Jev credential and explicit Save/Test/Remove. The source now mounts `JevAssistantSection` and `JevKeyControls` under those Settings destinations and removes the Connections Jev row. Board 06 frame 4 is retained as superseded art, not a route to build. The old Connections N10 entry remains in the sitemap only to preserve that frame's provenance. Browser/native navigation, secure-store failure and provider behavior remain unverified for the new placement.

Source review found the active Jev request only in `deltaAgent.ts` for ambiguous flashlight intent when the setting is enabled. The utterance is sent off-device; clear instructions stay local. Jev does not execute tools, and failure returns local clarification. The enable flag defaults off in SettingsStore JSON, while the Jev key uses `expo-secure-store` separately. Gemini's key still uses generic SettingsStore JSON, so neither moving the Jev field nor masking it resolves that separate storage gap. The broader `core/typesafe/` chat-triage and tool-dispatch modules are not imported by the active application path; the mobile `docs/TYPESAFE_INTEGRATION.md` now distinguishes that proposed architecture from the wired torch path. Phone/provider acceptance remains pending.
