...
OpenWakeWord is still absent on mobile. The current `wakeWordEngine.ts` matches transcript text only after recognition starts, so it cannot wake an idle app. A real on-device port needs 16 kHz PCM capture, ONNX feature extraction and calibrated phrase enrollment, plus Android microphone lifecycle and device false-positive/power checks. The desktop `onnxruntime-node` implementation is a reference for geometry and calibration, not a mobile runtime. Do not present phrase matching as acoustic standby until that path is implemented and verified.
