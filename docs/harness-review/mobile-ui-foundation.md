...
Delta Mobile 1.0.79/code135 source connects the conversation welcome orb, header mini orb, and composer mini orb to the existing Console runtime mode. Actual listening, generation/submission, playback and confirmation signals drive the visual state; measured microphone input may affect only listening scale. Reanimated runs the transforms and opacity on the UI thread, while reduced motion, hidden-screen and background states stop loops. Message avatars remain static. TypeScript, Android/web exports and arm64 debug build passed; on-device motion and interaction remain pending because the Pixel was locked during ARTEMIS inspection. See the mobile [orb state and motion chapter](../../../delta-mobile/docs/orb-state-and-motion.md) for ownership and the pending design review. No SDK hook or hardware contract changes.
