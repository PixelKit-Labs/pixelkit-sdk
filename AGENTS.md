...
3. **Observability on every function.** Wrap calls touching hardware, network, native modules, or filesystem in `traced(MODULE, 'op', fn, data)` from `packages/sdk/src/core/observability.ts`. Surface failures through an `error` field, never an empty `catch`. Use `tracedSafe` where failure is survivable. Trace completion events carry their own operation ID. Implicit event ownership is synchronous only: events after an `await` have no inherited trace ID. Carry explicit request IDs in event data for async application correlation; do not assume `getTrace(id)` includes every async callback.
...
- Keep optional providers optional: disabling Laya, missing model files, timeouts or provider errors must preserve the local harness, tool validation, capability checks and confirmation policy. Do not replace provider failures with fabricated model confidence or tool success.
